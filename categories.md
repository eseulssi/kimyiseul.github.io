...
- 채팅을 입력 중일 때 '입력중' 메시지 표출, 입력이 끝나면 채팅 내용 전송
- 언어 설정 시, 상대 유저의 언어를 자동으로 감지하여 내가 설정한 언어로 자동 번역, 여러 언어가 오가는 단체 채팅 특성 상, 자동 번역 기능이 유용할 것으로 예상
...
- 로젝트를 시작부터 배포까지 경험하면서 프로젝트의 라이프사이클을 이해하는 데 중요한 시간이었습니다. 이러한 경험은 실무에서의 개발자로서 필요한 역량에 도움을 줄 것입니다.
...
- 프로젝트를 기능별로 나눠 작업했기 때문에 HTML, CSS, Javascript, Spring Boot 등을 골고루 사용해야 했습니다. 특히 첫 프로젝트였기 때문에 컨벤션과 규칙을 정하는 데 일부 부족한 부분이 있었습니다.
